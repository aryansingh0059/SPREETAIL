...
router.get('/', async (req, res) => {
    try {
        ...
        const groups = await prisma_1.prisma.groupMembership.findMany({
            where: { userId },
            include: {
                group: {
                    include: {
...
router.post('/:id/members', async (req, res) => {
    try {
        ...
        const existingMembership = await prisma_1.prisma.groupMembership.findFirst({
            where: { groupId: id, userId: user.id }
        });
        let membership;
        if (existingMembership) {
            membership = await prisma_1.prisma.groupMembership.update({
            ...
            membership = await prisma_1.prisma.groupMembership.create({
                data: {
                    groupId: id,
                    userId: user.id,
                    joinDate: new Date(joinDate),
