...
import { runAnomalyEngine } from '../utils/anomalyDetector';
...
router.post('/upload', upload.single('file'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const anomalies = runAnomalyEngine(rawRows, activeMembers, group?.baseCurrencyCode || 'INR');
...
router.get('/:id/report', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const report = rows.map((row) => {
      ...
      const d: any = row.parsedData || row.rawData;
...
router.post('/:id/resolve-anomaly', async (req: AuthRequest, res: Response) => {
  try {
    ...
    let currentParsedData: any = anomaly.importRow.parsedData || anomaly.importRow.rawData;
