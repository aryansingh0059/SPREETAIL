...
export interface RawRow {
  id: string; // the database ID of the ImportRow
  data: any;  // the raw json
}
...
export const runAnomalyEngine = (
  ...
  for (const row of rawRows) {
    ...
    ].map((n: string) => n ? n : '').filter(n => n.length > 0);
    ...
    if (d.date && d.paid_by && d.amount) {
      ...
      if (!rowSignatures[sig]) rowSignatures[sig] = [];
